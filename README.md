...
<img src="Resources/ManyToMany.jpg" alt= "Many to Many Relationships" width="600" />

-   From image above, multiple tags are connected to multiple projects
-   In the database, we need an intermediary table (automatically generated by django). We will just create a projects table and a a tags table.
-   An intermediary table will automatically be created that stores the relationships.
-   Example from above; Project 1 has a tag 2

## Database Queries

<img src="Resources/Querysets.png" alt= "Query Set" width="600" />

-   Queries are usually in the views.py file
-   We will have a variable (queryset) that will hold the response... holds aa single object or a queryset (list of objects from the database).
-   We specify the ModelName; which is e.g the project, review or a tag.
-   Then we access the model objects attribute.
-   Then the method that we want to run. e.g if the ModelName is project, the above in the picture (.all()) will give us a query of all the projects in the database
-   For a single object; .get(), will get a single object based on an attribute.
-   For .filter(), if we dont add .filter(), it will do the same as .all(); will return all the items in that table. But we can specify an attribute and filter out that data.
-   .exclude() is the same thing as .filter() except it will exclude anything with the attribute name
